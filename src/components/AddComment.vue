<template>
  <div class="add-comment-container">
    <div class="tab-container">
      <ul>
        <li @click="isWrite = true"
          :class="isWrite ? 'active' : ''">Write</li>
        <li @click="renderMarkdown"
          :class="isWrite ? '' : 'active'">Preview</li>
      </ul>
      <div v-if="accessToken == null">
        <a :href="loginLink">Login</a>&nbsp;with GitHub
      </div>
    </div>
    <textarea :disabled="accessToken == null"
      v-if="isWrite"
      rows="5"
      v-model="newComment"
...
export default {
  ...
  computed: {
    ...mapGetters(["accessToken", "loginLink"]),
    commentBtnDisabled() {
      return (
  ...
  methods: {
    ...mapActions(["setAccessToken", "setTokenUser"]),
    renderMarkdown() {
